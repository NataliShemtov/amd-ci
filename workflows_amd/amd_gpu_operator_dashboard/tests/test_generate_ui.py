...
class TestBuildBundleInfo(TestCase):
    ...
    def test_catalog_regular_table_integrity(self):
        ...
        self.assertEqual(html.count("1.4.0"), 1)
        self.assertEqual(html.count("1.5.0"), 1)
        ...
        # Ensure that the entry with test_status FAILURE (gpu "1.6.0") does not appear.
        self.assertNotIn("1.6.0", html)

    def test_failed_only_combinations_displayed(self):
        """Test that combinations with only failed results are displayed as failed."""
        regular_results = [
            # Combination 1: GPU version 1.4.0 has both success and failure - should show as success
            {
            ...
            # Combination 2: GPU version 1.3.0 has only failures - should show as failed
            {
                OCP_FULL_VERSION: "4.18.1",
                GPU_OPERATOR_VERSION: "1.3.0",
                "test_status": STATUS_FAILURE,
        ...
        self.assertIn("1.4.0", html)
        self.assertIn("1.3.0", html)

        # 1.4.0 should be marked as successful (has success-link class)
        self.assertIn('class="success-link">1.4.0</a>', html)

        # 1.3.0 should be marked as failed (has failed-link class and (Failed) suffix)
        self.assertIn('class="failed-link">1.3.0 (Failed)</a>', html)

        # Should link to the correct URLs (latest success for 1.4.0, latest failure for 1.3.0)
    ...
    def test_mixed_success_failure_priority(self):
        ...
        self.assertNotIn('1.4.0 (Failed)', html)
...
class TestSemanticVersionValidation(TestCase):
    ...
    def test_gpu_version_with_suffix(self):
        ...
        result_with_suffix = {
            OCP_FULL_VERSION: "4.19.5",
            GPU_OPERATOR_VERSION: "1.4.0(bundle)",
            "test_status": STATUS_SUCCESS,
            "prow_job_url": "https://example.com/job1",
            "job_timestamp": "1712345678"
        }
...
class TestGenerateTestMatrix(TestCase):
    ...
    @patch('workflows_amd.amd_gpu_operator_dashboard.generate_ci_dashboard.load_template')
    def test_generate_test_matrix_with_separated_structure(self, mock_load_template):
        ...
        mock_load_template.side_effect = [
            '<html><head><title>AMD GPU Operator Test Matrix</title></head><body>',  # header.html
        ...
        self.assertIn('</html>', html_result)
        self.assertIn('<title>AMD GPU Operator Test Matrix</title>', html_result)

        # Verify OCP versions are processed (sorted in reverse order)
        self.assertIn('4.18', html_result)
        self.assertIn('4.19', html_result)

        # Verify release test results appear in the table
        self.assertIn('1.4.0', html_result)
        self.assertIn('1.5.0', html_result)
        ...
        # Verify notes appear
        self.assertIn('Test note for 4.18', html_result)
    ...
    def test_generate_test_matrix_with_empty_sections(self, mock_load_template):
        ...
        self.assertIsInstance(html_result, str)
        self.assertIn('<html>', html_result)
        self.assertIn('4.20', html_result)
    ...
    def test_generate_test_matrix_filters_invalid_versions(self, mock_load_template):
        ...
        self.assertNotIn('1.3.0', html_result)
