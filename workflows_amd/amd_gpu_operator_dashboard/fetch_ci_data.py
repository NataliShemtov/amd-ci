...
from workflows_amd.common.utils import logger
...
TEST_RESULT_PATH_REGEX = re.compile(
    ...
    r"(?P<ocp_version>\d+\.\d+)-stable-amd-gpu-operator-e2e-(?P<gpu_version>\d+-\d+-x|master))/"
    r"(?P<build_id>[^/]+)"
)
...
def extract_build_components(path: str) -> Tuple[str, str, str, str]:
    ...
    if not match:
        msg = "AMD GPU operator path regex mismatch" if "amd-gpu-operator-e2e" in original_path else "Unexpected path format"
...
def filter_gpu_finished_files(all_finished_files: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], Dict[Tuple[str, str, str], Dict[str, Dict[str, Any]]]]:
    ...
    - If nested finished.json exists (artifacts/amd-gpu-operator-e2e-{gpu_suffix}/gpu-operator-e2e/finished.json), use it
    ...
    for file_item in all_finished_files:
        ...
        # Check if it's an AMD GPU operator E2E finished.json file
        if not ("amd-gpu-operator-e2e" in path and path.endswith('/finished.json')):
            continue

        # Determine file type and extract build key
        is_nested = '/artifacts/amd-gpu-operator-e2e-' in path and '/gpu-operator-e2e/finished.json' in path
...
def build_files_lookup(
    ...
    for file_item, file_type in all_files_with_type:
        ...
        # Skip non-AMD GPU operator paths early
        try:
            repo, pr_number, job_name, build_id = extract_build_components(path)
        except ValueError:
            continue
...
def process_single_build(
    ...
    # Check for mismatch between nested AMD GPU operator test and top-level build result
    if dual_builds_info and key in dual_builds_info:
        ...
        if 'nested' in dual_files and 'top_level' in dual_files:
            ...
            # Warn if AMD GPU operator succeeded but overall build failed
            if nested_status == STATUS_SUCCESS and top_level_status != STATUS_SUCCESS:
                logger.warning(
                    f"Build {build_id}: AMD GPU operator tests SUCCEEDED but overall build has finished with status {top_level_status}."
                )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="AMD GPU Operator Test Matrix Utility")
    ...
    if os.path.exists(args.baseline_data_filepath):
        ...
        logger.info(f"No baseline data file found at {args.baseline_data_filepath}, starting fresh")
