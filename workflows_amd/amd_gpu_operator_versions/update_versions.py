...
from workflows_amd.amd_gpu_operator_versions.settings import Settings
from workflows_amd.amd_gpu_operator_versions.openshift import fetch_ocp_versions
from workflows_amd.amd_gpu_operator_versions.version_utils import get_latest_versions, get_earliest_versions
from workflows_amd.amd_gpu_operator_versions.amd_gpu_operator import get_operator_versions, get_sha
...
def create_tests_matrix(diffs: dict, ocp_releases: list, gpu_releases: list) -> set:
    ...
    - New main/development branch changes (tested on newest and oldest OCP)
    - New OpenShift versions (tested against all GPU operator releases)
    ...
    Args:
        ...
        gpu_releases: List of GPU operator release versions
    ...
    if "gpu-main-latest" in diffs:
        logger.info('Main branch SHA changed, adding tests for newest and oldest OCP')
...
def main():
    ...
    gpu_releases = list(gpu_versions.keys())  # Test all GPU operator versions
    
    logger.info(f'Generating tests for {len(ocp_releases)} OCP releases and {len(gpu_releases)} GPU releases')
